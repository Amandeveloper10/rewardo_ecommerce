    ...
    <!-- End Inlile Menu -->
        ...
        <div class="row gy-3">
			...
			<div class="col-md-6">
                <div class="card h-100">
                    <div class="card-body">
                        <h5 class="mb-4 text-uppercase d-flex">{{\App\CPU\translate('Redo Pay')}}</h5>

                        @php($config=\App\CPU\Helpers::get_business_settings('redo_pay'))
                        <form action="{{route('admin.business-settings.payment-method.update',['redo_pay'])}}"
                              style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                              method="post">
                            @csrf
                            @if(isset($config))
                                <label
                                    class="mb-3 d-block font-weight-bold title-color">{{\App\CPU\translate('Redo Pay Login Details')}}</label>
                                <div class="form-group">
                                    <label
                                        class="d-flex title-color">{{\App\CPU\translate('Login')}} {{\App\CPU\translate('ID')}} </label>
                                    <input type="text" class="form-control" name="login_id"
                                           value="{{env('APP_MODE')=='demo'?'':$config['login_id']}}">
                                </div>
                                <div class="form-group">
                                    <label
                                        class="d-flex title-color">{{\App\CPU\translate('password')}}</label>
